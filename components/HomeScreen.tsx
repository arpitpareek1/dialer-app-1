...
export default function HomeScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('loggedInUser').then(setUsername);
  }, []);

  const dialPhoneNumber = async () => {
    if (number.length > 0) {
      // Dial the number
      make_call(number);
      // Log call info
      ...
      try {
        const prev = await AsyncStorage.getItem('callLogs');
        let logs = prev ? JSON.parse(prev) : [];
        logs.push(callInfo);
